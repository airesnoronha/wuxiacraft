...
public class SkillsGui extends GuiScreen {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0) {
            ...
            int offsetLeft = Math.max(0, offset - displayTS);
            int displayKS = skillCap.getKnownSkills().size() + displayTS - offsetLeft;
            for (i = i; i < maxi && i < displayKS; i++) {
                if (inBounds(mouseX, mouseY, this.guiLeft + 8, this.guiTop + 17 + i * 17, 9, 9)) {
                    removeSkill(skillCap.getKnownSkills().get(offsetLeft + i - displayTS));
                }
                if (inBounds(mouseX, mouseY, this.guiLeft + 20, this.guiTop + 17 + i * 17, 9, 9)) {
                    selectSkill(skillCap.getKnownSkills().get(offsetLeft + i - displayTS));
                }
            }
            ...
            if (skillCap.getKnownSkills().size() + cultTech.getTechniqueSkills().size() > maxi) {
                if (inBounds(mouseX, mouseY, this.guiLeft + 186, this.guiTop + 104, 9, 9)) {
                    int max = skillCap.getKnownSkills().size() + cultTech.getTechniqueSkills().size() - maxi;
                    this.offset = Math.min(max, this.offset + 1);
                }
            }
        }
    }
    ...
    private void drawBackgroundLayer() {
        ...
        for (i = i; i < maxi && i < displayKS; i++) {
            ...
            if (skillCap.getSelectedSkills().contains(skillCap.getKnownSkills().get(offsetLeft + i - displayTS))) {
                drawTexturedModalRect(14, 2, 45, 148, 9, 9);
            } else {
                drawTexturedModalRect(14, 2, 36, 148, 9, 9);
            }
    ...
    private void drawSkillsIcons() {
        ...
        for (i = i; i < maxi && i < displayKS; i++) {
            ...
            GlStateManager.translate(31, 16 + 17 * i, 0);
            GlStateManager.scale(11f / 256f, 11f / 256f, 1f);
            String UName = skillCap.getKnownSkills().get(offsetLeft + i - displayTS).getUName();
            ResourceLocation tex = skillIcons.get(UName);
            this.mc.getTextureManager().bindTexture(tex);
            drawTexturedModalRect(0, 0, 0, 0, 256, 256);
            GlStateManager.popMatrix();
    ...
    private void drawForegroundLayer() {
        ...
        for (i = 0; i < maxi && i < displayTS; i++) {
            ...
            int color = skillCap.getSelectedSkills().contains(cultTech.getTechniqueSkills().get(offset + i)) ? 0x2C6CFF : 0xFFFFFF;
            this.fontRenderer.drawString(cultTech.getTechniqueSkills().get(offset + i).getName(), 0, 0, color);
            GlStateManager.popMatrix();
        }
        int offsetLeft = Math.max(0, offset - displayTS);
        int displayKS = skillCap.getKnownSkills().size() + displayTS - offsetLeft;
        for (i = i; i < maxi && i < displayKS; i++) {
            ...
            int color = skillCap.getSelectedSkills().contains(skillCap.getKnownSkills().get(offsetLeft + i - displayTS)) ? 0x2C6CFF : 0xFFFFFF;
            this.fontRenderer.drawString(skillCap.getKnownSkills().get(offsetLeft + i - displayTS).getName(), 0, 0, color);
            GlStateManager.popMatrix();
        }
        GlStateManager.popMatrix();
    }
