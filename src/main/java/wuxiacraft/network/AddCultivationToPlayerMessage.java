		...
		if (sideReceived.isServer()) {
			ctx.enqueueWork(() -> {
				...
				if (stats.getEnergy() >= message.energy && kt != null) {
					...
					if (essenceStats.getLevel() == CultivationLevel.DEFAULT_ESSENCE_LEVEL) energy_conversion *= 6;
					double added_base = message.energy * energy_conversion * (message.special ? 1.1 : 1);
					stats.addBase(added_base);
					essenceStats.addEnergy(-message.energy);
					divineStats.addEnergy(-message.energy * 0.2);
